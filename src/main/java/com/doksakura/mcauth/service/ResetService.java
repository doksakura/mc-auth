...
public class ResetService {
    ...
    public String reset(Validate validate) throws AttributeNotFoundException, JsonProcessingException {
        ...
        mailFormat.setPersonal("MC Doksakura");
        mailFormat.setHeader("Reset Password Confirmation - MC Doksakura");
        mailFormat.setHtml(kafkaService.mailFormatString.replace("${email}", player.get().getEmail().split("@")[0]).replace("${name}", validate.getName()).replace("${link}", link));
